...
function userRequestHandler(req,userRes){
    ...
    */
    ...
    function REVERSE_PROXY(callback) {
      if (userRule.testReverseProxy) {
          userRule.testReverseProxy(req, resourceInfo, callback);
      } else {
          callback();
      }
    }

    function PREPARE_USE_LOCAL_TEST(callback) {
        if (userRule.prepareUseLocalTest) {
          userRule.prepareUseLocalTest(req, resourceInfo, callback);
        } else {
          callback();
        }
    }
...
function connectReqHandler(req, socket, head){
    ...
    resourceInfoId = global.recorder ? global.recorder.appendRecord(resourceInfo) : null;
